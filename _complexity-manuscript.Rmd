...
Figure \@ref:(fig:attractors) shows the participant's four major repeating profiles, which correspond to the coloured nodes in Figure \@ref(fig:recnet). We can observe that about a fifth of the participant's responses fall into a relatively balanced profile, while `r ((7+8)/87 * 100) %>% round(., digits = 0)`% (second and fourth profile combined) indicate what self-determination theory would consider an "optimal" motivation profile -- high in autonomous forms of motivation and low in extrinsic ones.

```{r attractors, fig.width = 11.69/2, fig.cap = "Main attractors corresponding to the colors indicated in the previous plot. See text and supplementary website (section xxx) for a thorough exposition."}
