...
The first step of the analysis is to plot the data points with their distances to all other data points in a matrix, as shown in Figure \@ref(fig:rqa-pedagogical-distance). After this, the figure can be "coarse-grained" with a rule, that binarises each cell into recurring (black) or not (white). The rule in Figure \@ref(fig:rqa-pedagogical-distance) is "preserve recurrences with absolute distance of 1 or less".
