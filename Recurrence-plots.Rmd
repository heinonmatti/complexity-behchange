...
It is also possible to compute a variety of network indices from the transition network.

```{r}
attractor_plots <- attractor_plots %>% 
  dplyr::mutate(centralities = purrr::map2(.x = relative_freqs_networks,
                                           .y = User,
                                           .f = ~qgraph::centralityPlot(.x, weighted = TRUE, signed = FALSE)))

# attractor_plots$centralities
```

## Robustness to recurrence rate

Here we perform the same analysis, but with recurrence rate of 0.10 instead of 0.05.

Here's the 6-dimensional motivation system's recurrence plot, weighted by similarity.

```{r firstlast-weighted-rr}
set.seed(100)
...
```{r firstlast-attractor-extraction-rr}
# Get number of maximally connected node
emadata_nested_wrangled_both_recnets_nodes <- emadata_nested_wrangled_both_recnets %>% 
  dplyr::mutate(max_connected_day = purrr::map(.x = graph_from_adjacency,
                                     .f =  ~which.max(igraph::degree(.x))
