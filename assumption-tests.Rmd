...
Source code for this document is found [here](https://raw.githubusercontent.com/heinonmatti/complexity-behchange/master/data/assumption-tests.Rmd).
...
# readr::write_tsv(edidata, path = "./data/data_20p_9var_plus_time.tsv")
mydata <- readr::read_tsv("./data/data_20p_9var_plus_time.tsv")
...
In the tables below, where exact p-values are not reported, we choose the threshold of p < `r alphaLevel` to reject the test. The rationale for this is that we consider what's presented as the "Overview table" of four tests to consist of a family of tests, hence 0.05 / 4 = 0.0125. We also present the tables using an alpha level of 0.05 / 12 = `r 0.05/12 %>% round(., 6)`. Ultimately, many different choices would be justified, and we encourage the reader to do their own analysis with the shared data. Obviously, not correcting for multiple testing and using 0.05 as the cutoff, would reject much more time series. **Note: the KPSS test is built in such a way that it does not produce p-values < 0.01, hence interpretation is not sensible for alpha levels below that.**
...
# Percentage of time series tests rejected in segments based on n {.tabset}
...
Interpretation instructions: One time series is one variable in one person's data. The top-left number indicates the number of rejections of the test indicated by row name, in the variable indicated by column name. The total possible number of tests would be the number of collected time series of that variable, that is, the number of individuals in the dataset (20).
